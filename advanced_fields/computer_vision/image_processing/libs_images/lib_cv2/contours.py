...
img = cv2.imread(path)  # note that OpenCV uses BGR (and not RGB)
...
cv2.imwrite('img_' + path, img)
cv2.imwrite('bgr_' + path, img_bgr)
cv2.imwrite('rgb_' + path, img_rgb)
cv2.imwrite('bw_' + path, img_bw)
cv2.imwrite('cnt_' + path, img_cont)
