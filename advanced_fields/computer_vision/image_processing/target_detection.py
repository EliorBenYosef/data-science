...
img = cv2.imread('../../../datasets/per_field/cv/target_detection/tarpsB.jpg', cv2.IMREAD_COLOR)
# convert to CIELab
...
colors = [([91, 40, 40]), ([40, 209, 255]), ([81, 60, 166])]
# rough conversion of BGR target values to CIELab
cielab_colors = [([20, 20, -40]), ([80, 0, 90]), ([40, 70, 30])]
...
for cielab_color in cielab_colors:
    ...
    cv2.floodFill(img, ff_mask, minLoc, 255, (12, 12, 12), (12, 12, 12), cv2.FLOODFILL_MASK_ONLY)
    ff_mask = ff_mask[1:-1, 1:-1]
    contours, hierarchy = cv2.findContours(ff_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    # Find the index of the largest contour
    areas = [cv2.contourArea(c) for c in contours]
    max_index = np.argmax(areas)
    cont = contours[max_index]
    print('target color = {}'.format(img[minLoc[1], minLoc[0], :]))
    # putting text and outline rectangles on image
    x, y, w, h = cv2.boundingRect(cont)
    cv2.rectangle(img, (x, y), (x + w, y + h), colors[i], 2)
    cv2.putText(img, targ[i], (x - 50, y - 10), cv2.FONT_HERSHEY_PLAIN, 0.85, (0, 255, 0))
...
cv2.imwrite('output_img/target_detection.jpg', img)
