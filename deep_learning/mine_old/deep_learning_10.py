...
def plot_model_accuracy(trained_model):
    ...
    plt.show()
    fig.savefig('results/DL10_model_accuracy.jpg')
    plt.close(fig)
...
def plot_model_loss(trained_model):
    ...
    plt.show()
    fig.savefig('results/DL10_model_loss.jpg')
    plt.close(fig)
...
def display_activations(layers_names, layers_activations, layers_filters, enhanced=True):  # images_per_row
    ...
    for layer_name, layer_activation, cols in zip(layers_names, layers_activations, layers_filters):  # Displays the feature maps
        ...
        if enhanced:
            ...
            plt.show()
            fig.savefig('results/DL10_activations_' + layer_name + '_enhanced.jpg')
            plt.close(fig)
            ...
            plt.show()
            fig.savefig('results/DL10_activations_' + layer_name + '_regular.jpg')
            plt.close(fig)
...
plt.show()
fig.savefig('results/DL10_image.jpg')
plt.close(fig)
