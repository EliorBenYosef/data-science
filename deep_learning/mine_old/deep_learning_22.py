...
def plot_model_accuracy(trained_model, title, ticks=0):
    ...
    plt.show()
    fig.savefig('DL22_model_accuracy_' + title + '.jpg')
    plt.close(fig)
...
def plot_model_loss(trained_model, title, ticks=0):
    ...
    plt.show()
    fig.savefig('DL22_model_loss_' + title + '.jpg')
    plt.close(fig)
...
circle = np.expand_dims(cv2.imread('deep_learning_22_circle_28.jpg', cv2.IMREAD_GRAYSCALE), 2)
square = np.expand_dims(cv2.imread('deep_learning_22_square_28.jpg', cv2.IMREAD_GRAYSCALE), 2)
